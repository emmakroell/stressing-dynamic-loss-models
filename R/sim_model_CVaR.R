#' Simulate the stressed model with CVaR constraints
#'
...
#' @export
stressed_sim_CVaR <- function(kappa, jump_dist, stress_type = "CVaR",
    ...
    withr::with_seed(720,{
      ...
      if (is.null(stress_parms$q) & !(is.null(stress_parms$VaR_stress))){
        stress_parms$q <- stress_parms$VaR_stress *
          compute_VaR(kappa=kappa,c=stress_parms$c,dist=jump_dist)
      }
      if (is.null(stress_parms$s) & !(is.null(stress_parms$CVaR_stress))){
        stress_parms$q <- stress_parms$CVaR_stress *
          compute_CVaR(kappa=kappa,q=stress_parms$q,c=stress_parms$c,dist=jump_dist)
      }

      eta <- eta_CVaR(kappa=kappa, stress_parms=stress_parms, dist=jump_dist)
      ...
      for (i in 1:Nsteps){
        distorted <- sim_G_kappa_CVaR(t=times[i],x=x_seq,eta=eta,kappa=kappa,
                                      stress_parms=stress_parms,dist=jump_dist,
                                      Draws=Draws)
...
#' compute G^Q and kappa^Q for Y ~ dist for CVaR
...
#'
#' @export
sim_G_kappa_CVaR <- function(t,x,eta,kappa,stress_parms,dist,Draws,
                        delta_y=0.1,y_max=100,tol=1e-10) {
  with(c(dist,stress_parms), {
    # x is a vector
    xlen <- length(x)
    y_max <- seq(1,y_max)[which(dens_fun(seq(1,y_max),parms) < tol)[1]]
    y <- seq(0,y_max,by=delta_y)  # h is constant for x+y > q
    ylen <- length(y)
    h <- array(dim=c(xlen,ylen))

    # compute h at each y
    for (i in 1:ylen){
      h[,i] <- h_CVaR(x=x,y=y[i],t=t,dist=dist,eta=eta,q=q,kappa=kappa)
    }
